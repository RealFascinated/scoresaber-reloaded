...
export default function Navbar() {
  ...
  return (
    <nav
      className={cn(
        "sticky inset-x-0 top-0 w-screen px-5 py-1 flex justify-around items-center backdrop-blur-md border-b border-muted transition-all transform-gpu select-none z-50",
...
function SimpleNavLink({ content, href }: { content: ReactElement; href: string }) {
  return (
    <Link
      className="flex gap-3 items-center hover:opacity-80 transition-all transform-gpu"
      href={href}
