...
export default function ScoreSaberPlayerMedalScores({ player }: ScoreSaberPlayerMedalScoresProps) {
  ...
  useDocumentTitle(
    ssrConfig.siteTitleTemplate.replace(
      "%s",
      `${player.name} / Medals / ${page} / ${SORT_OPTIONS.find(s => s.value === sort)?.name} / ${capitalizeFirstLetter(direction)}`
    )
  );
  ...
  const {
    ...
    queryFn: async () => {
      const response = await ssrApi.fetchPlayerMedalScores(
        player.id,
        page,
        {
          field: sort,
  ...
  const buildUrl = useCallback(
    (pageNum: number) => {
      ...
      return `/player/${player.id}/medals?${queryString}`;
    },
