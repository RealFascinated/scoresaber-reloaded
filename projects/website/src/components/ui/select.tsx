...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      className={cn(
        "border-muted/50 ring-offset-background placeholder:text-muted-foreground focus:ring-ring hover:border-primary/50 bg-muted flex h-9 w-full cursor-pointer items-center justify-between rounded-[var(--radius-md)] border px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-colors duration-200 focus:ring-1 focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
        className
      )}
      {...props}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        className={cn(
          "bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 border-muted/50 relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-[var(--radius-md)] border shadow-md",
          position === "popper" &&
