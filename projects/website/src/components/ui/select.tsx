...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      className={cn(
        "border-muted/50 ring-offset-background placeholder:text-muted-foreground focus:ring-ring hover:border-primary/50 shadow-xs focus:outline-hidden flex h-9 w-full cursor-pointer items-center justify-between whitespace-nowrap rounded-md border bg-transparent px-3 py-2 text-sm transition-all focus:ring-1 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
        className
      )}
      {...props}
    >
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      className={cn(
        "focus:bg-accent focus:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 relative flex w-full cursor-pointer select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm focus:outline-none",
        className
      )}
      {...props}
    >
