...
export const steamAuthPlugin = (cfg: SteamAuthPluginOptions): BetterAuthPlugin => ({
  id: "steamAuthPlugin",
  endpoints: {
    ...
    steamCallback: createAuthEndpoint("/steam/callback", { method: "GET" }, async ctx => {
      if (!ctx?.request?.url) {
        return Response.redirect(`${`${ctx.context.baseURL}/error`}?error=missing_request_url`);
      }
      ...
      if (!steamid) {
        return Response.redirect(`${`${ctx.context.baseURL}/error`}?error=steamid_missing`);
      }
      ...
      const url = new URL("/home", hostname);
