...
async function removeCompoundIndex() {
  try {
    ...
    Logger.info("Current indexes:", indexes.map(idx => idx.key));
    
    // Find and remove the compound index
    const compoundIndexName = indexes.find(idx => 
    ...
    Logger.info("Remaining indexes:", remainingIndexes.map(idx => idx.key));
