...
import { scoreSaberScoresTable, type ScoreSaberScoreRow } from "backend/db/schema";
import { sql } from "drizzle-orm";
import { ScoreSaberScoreModel } from "../src/model/score/impl/scoresaber-score";

dotenv.config();

/** Lean Mongo document from `scoresaber-scores` collection. */
type MongoScoreSaberScoreLeanDoc = {
  _id?: unknown;
  scoreId?: unknown;
  playerId?: unknown;
...
/** Maps Mongo doc → Drizzle insert row (same columns as `scoreSaberScoresTable` inserts in the backend score pipeline). */
function mongoScoreSaberDocToRow(
  doc: MongoScoreSaberScoreLeanDoc
): typeof scoreSaberScoresTable.$inferInsert {
  const id = parsePositiveIntId(doc);
  if (id == null) {
    throw new Error("missing or invalid _id / scoreId");
  }
...
async function flushBatch(rows: (typeof scoreSaberScoresTable.$inferInsert)[]): Promise<void> {
  if (rows.length === 0) {
    return;
  }
...
async function main() {
  ...
  const batch: (typeof scoreSaberScoresTable.$inferInsert)[] = [];

  const cursor = ScoreSaberScoreModel.find().lean().cursor();
  ...
  async function flush() {
    ...
    try {
      ...
      errors += chunk.length;
      Logger.error(`[mongo-to-postgres] scoresaber-scores batch failed (${chunk.length} rows): ${e}`);
    }
  }
  ...
  for await (const doc of cursor) {
    ...
    try {
      batch.push(mongoScoreSaberDocToRow(doc as MongoScoreSaberScoreLeanDoc));
      ...
      errors++;
      Logger.error(
        `[mongo-to-postgres] scoresaber-scores map failed _id=${String((doc as MongoScoreSaberScoreLeanDoc)._id)}: ${e}`
      );
    }

    if (processed % 5000 === 0) {
      Logger.info(
        `[mongo-to-postgres] scoresaber-scores progress processed=${processed} upserted=${upserted} errors=${errors} (batchSize=${batchSize})`
      );
    }
  }
  ...
  Logger.info(
    `[mongo-to-postgres] scoresaber-scores done processed=${processed} upserted=${upserted} errors=${errors}`
  );
}
